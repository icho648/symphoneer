...
.github/
  workflows/
    check.yml                PR / main 上运行 pnpm check
.nvmrc                       本地与 CI 共用的 Node 主版本
.symphoneer/
...
scripts/
  check-project.mjs          链接、Agent 导航、Plan、测试位置、依赖和 Codemap 路径检查
  dev.ts                     Runtime 与 Web 的前台 launcher（`pnpm dev`）
tests/
  contracts/                 共享 Schema 与 Agent Runner contract
  runtime/                   Scheduler、Workflow、JSONL、Runtime 命令、HTTP / SSE、launcher
...
当前没有 workspace 安装布局、`packages/` 或 `apps/` 目录；Runtime、CLI、Web 各有进程边界 manifest，`src/contracts`、`src/runtime` 和 `src/runtime-client` 通过根 `package.json` 的 `link:` 依赖暴露为 `@symphoneer/contracts`、`@symphoneer/runtime` 和 `@symphoneer/runtime-client`，依赖仍由根统一安装，因此只有根 `node_modules`；`pnpm-workspace.yaml` 只记录依赖构建脚本的授权决定。没有数据库、队列、部署配置或生成流水线；CI 仅运行 `pnpm check`。Runtime 持久化使用 Host 注入的数据目录，Web 通过 loopback HTTP / SSE 访问 Runtime，`/healthz` 直接报告 Runtime 进程的 PID、启动时间和运行时长；完整浏览器人工审查、真实安装目录发现、GitHub 网络和 Codex 真实 Turn 仍未验证。
...
scripts/dev.ts ─────> Node stdlib（产品级前台 launcher）
...
- `scripts/dev.ts` 是产品级前台 launcher：复用已健康的 Runtime、显式退出时才向子进程转发停止信号，并由根 `tsc` 类型检查。
