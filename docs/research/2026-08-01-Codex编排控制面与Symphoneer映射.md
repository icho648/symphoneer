...
- **Decision：** Tracker 保存 Task 原生状态；Symphony Runtime 负责调度；Symphoneer 保存
...
- 本文不是 Codex 官方规范，也不把原文中的能力清单当成 Symphoneer 的产品范围。
...
## 对象模型：不要把 Codex 对象当成 Symphoneer 业务对象

| Codex 概念 | 原文中的作用 | Symphoneer 中的建议映射 |
|---|---|---|
...
核心边界：**Tracker 保存 Task 原生状态；Symphoneer 保存 Task 引用、Attempt、
...
对 Symphoneer 的直接结论：`Workspace` 应记录路径、来源、宿主机、分支和隔离方式；不能只显示一个抽象的“Agent 正在运行”。
...
- 复杂 DAG 的依赖、重试和节点状态应放在外部 Registry/Symphoneer，不应靠翻聊天记录恢复。

### 当前明确不吸收的范围

- 不因为文章列出多种拓扑，就把 Symphoneer 做成通用 Multi-Agent Workflow Engine。
- 不在 V1 同时支持多个 Runtime、多个 Tracker 或多个模型 Adapter。
- 不把 Subagent 数量当作产品价值；没有独立证据和可量化收益时，单 Agent + 工具更合适。
- 不让 MCP 取代 App Server 的运行事件，也不让 MCP 成为 Symphoneer 的业务状态库。
...
3. **Symphoneer** 负责 Task/Attempt/Verification/Review 的业务投影、人工控制和证据关联。
