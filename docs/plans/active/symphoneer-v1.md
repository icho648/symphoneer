...
- [x] 2026-08-03 Issue #13 PR 复审：修复 `before_run` 失败后的 Workspace 所有权回退、同一 Attempt / Thread 的连续 Turn 推进、容量等待误增连续失败次数、失败校验破坏 Turn 所有权原子性、reconciliation 部分写入，以及成功 Attempt 携带 failure；定向回归和全量检查通过。
